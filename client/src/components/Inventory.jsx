        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 text-gray-500 gap-2">
              ...
              <span className="text-sm">Inventaire vide — ouvre des caisses!</span>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="p-3 rounded-xl border-2 bg-white/[0.03] hover:bg-white/[0.06] transition-all group"
                  style={{ borderColor: item.rarity_color + '55' }}
                >
                  {item.skin_image && (
                    <img
                      src={item.skin_image}
                  ...
                  <p className="text-xs font-bold truncate" style={{ color: item.rarity_color }}>
                    {item.skin_name}
                  </p>
                  <p className="text-[10px] font-semibold mb-2" style={{ color: item.rarity_color + '99' }}>
                    {item.rarity}
                  </p>
                  <button
                    onClick={() => handleSell(item)}
                    disabled={sellingId === item.id}
                    className="w-full flex items-center justify-center gap-1 bg-gradient-to-br from-yellow-500/80 to-amber-600/80 text-dark-900 font-bold py-1.5 rounded-lg text-xs hover:-translate-y-0.5 hover:shadow-[0_3px_10px_rgba(245,166,35,0.3)] transition-all disabled:opacity-50"
                  >
